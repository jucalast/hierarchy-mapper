...
class ChatOrchestrator:
                ...
                if payload.thread_id:
                    try:
                        ...
                        await save_message(owned_session, thread_id=payload.thread_id, role="assistant",
                                           content=response_text, ui_module="DealStatus", data=data,
    ...
    async def _deal_status_handler(
        ...
        if log_queue:
            ...
            try:
                ...
                act_subs = [f"{a.get('subject')} ({a.get('due_date')})" for a in activities[:5]]
                ...
                DEAL_STATUS_NARRATIVE_PROMPT = f"""Você é um Diretor de Vendas B2B Senior. Sua missão é narrar o ESTADO REAL do negócio.
        
                FATOS PARA NARRATIVA:
                - Negócios: {deal_info or 'Nenhum'}
                - Atividades: {", ".join(act_subs) or 'Nenhuma registrada'}
                - Comunicações: {num_emails} threads de e-mail, {num_wa} conversas de WhatsApp. Última interação: {last_interaction}.
                - Notas: {note_snippet}

                REGRAS DE OURO:
                1. Seja direto, executivo e use nomes/valores reais.
                2. "FALA E MOSTRA": Gere frases curtas que antecipem o que o usuário vai ver nos cards.
                3. FOCO: Fatos -> Implicação -> Próxima ação sugerida.
                4. PROIBIDO inventar dados.

                Retorne JSON: 
                {{"intro": "Análise do pipeline e valores envolvidos.", "tasks": "O que as atividades atuais revelam sobre a velocidade do negócio.", "analysis": "Insight final cruzando o histórico de comunicação com o CRM."}}"""
                ...
                if activities:
                    ...
                    log_queue.put_nowait({"type": "data_found", "entity": "activities_group", "data": activities[:5]})
                    await asyncio.sleep(0.5)
        ...
        calc = ctx.get("calculated_status", {}) or {}
        ...
        if activities:
            ...
            for a in activities[:6]:
                ...
                lines.append(f"  - {a.get('due_date', '?')}: {a.get('subject')} [{done}]")
        ...
        if calc:
            lines.append(f"\nMÉTRICA DE SILÊNCIO: {calc.get('summary', '')}")
        ...
        try:
            ...
            res = await ask_llm(
                DEAL_STATUS_PROMPT.format(context=context_block, today=today_str),
            ...
            response_data = {"analysis": f"Não consegui gerar o briefing neste momento ({e}). Dados brutos: {context_block[:300]}"}

        # Consolida a narrativa para o campo 'response' que o front espera
        narrative_parts = []
        if response_data.get("intro"): narrative_parts.append(response_data["intro"])
        # Suporta tanto 'tasks' quanto 'tasks_analysis' (variantes de prompt)
        t_val = response_data.get("tasks") or response_data.get("tasks_analysis")
        if t_val: narrative_parts.append(t_val)
        if response_data.get("analysis"): narrative_parts.append(response_data["analysis"])

        # Se não tiver nada, usa o fallback do briefing básico
        combined_response = "\n\n".join(narrative_parts) if narrative_parts else response_data.get("analysis", "Status processado.")

        # ── AÇÕES SUGERIDAS ──────────────────────────────────────────────────
        # Geradas mecanicamente a partir do contexto (sem chamada LLM extra).
        wa_results = ctx.get("whatsapp_result", {}).get("resultado", {}).get("messages_by_contact", [])
        email_results = ctx.get("email_result", {}).get("resultado", {}).get("messages_by_contact", [])
        contacts = ctx.get("pipedrive_details", {}).get("persons", []) or []
        org_name = org.get("name", "empresa")
        ...
        return {
            **response_data,
            ...
            "_internal_context": ctx  # Para caching entre mensagens
        }
    ...
    async def _build_context_with_intent(
        ...
        internal_context: Dict[str, Any] = {}
        if query_type == "enrichment":
            osint_context = await execute_osint_enrichment(intent_info, org_id, session, log_queue=log_queue)
            internal_context.update(osint_context)
