...
const findContactByNumber = async (client, number) => {
    const tryFind = async (num) => {
        try {
            const formatted = num.includes('@c.us') ? num : `${num}@c.us`;
            const contact = await client.getContactById(formatted);
            if (contact && contact.id) return contact;
        } catch (e) {}
        return null;
    };

    let raw = number.replace('@c.us', '').replace(/\D/g, '');
    // Garante prefixo 55 se for brasileiro sem DDI
    if (raw.length === 10 || raw.length === 11) {
        if (!raw.startsWith('55')) raw = `55${raw}`;
    }

    let contact = await tryFind(raw);
    
    // Se não encontrou e for número brasileiro, tenta alternar o 9º dígito
    if (!contact && raw.startsWith('55')) {
        let altNumber;
        if (raw.length === 13) {
            altNumber = `55${raw.substring(4)}`; // Remove o 9
        } else if (raw.length === 12) {
            altNumber = `55${raw.substring(2,4)}9${raw.substring(4)}`; // Adiciona o 9
        }
        if (altNumber) {
            contact = await tryFind(altNumber);
        }
    }
...
const getProfilePic = async (client, contactId) => {
    ...
    let formatted = contactId.includes('@c.us') ? contactId : `${contactId}@c.us`;
    let url = await tryGet(formatted);
    
    // Fallback p/ número brasileiro (tenta com/sem 9º dígito e com prefixo 55 se faltar)
    if (!url) {
        let raw = formatted.replace('@c.us', '').replace(/\D/g, '');
        if (raw.length === 10 || raw.length === 11) {
            if (!raw.startsWith('55')) raw = `55${raw}`;
        }
        ...
        if (raw.startsWith('55')) {
            ...
            if (altNumber) {
                url = await tryGet(`${altNumber}@c.us`);
            }
        }
    }
