...
import styles from '@/features/network-graph/styles/NodeStyles.module.css';
import {
  MapPin,
  GraduationCap,
  Briefcase,
          ...
          <div className={styles.nodeAvatar}>
            {effectiveLevel === 0 ? (
              <img 
                ...
                style={{ objectFit: "contain", background: "var(--sw-surface-raised)" }} 
                loading="lazy" 
                decoding="async" 
                onError={(e) => { 
                  ...
                  if (!target.src.includes("ui-avatars")) { 
                    target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(fallbackName)}&background=818cf8&color=fff`; 
                  } 
                }} 
              />
            ) : data.linkedin ? (
